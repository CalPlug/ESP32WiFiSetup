...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  while(1){
    ...
    if (connect) {
      connect = false;
        DEBUG_WM("INSIDE WHILE INSIDE IF");
      delay(2000);
...
boolean WiFiManager::captivePortal() {
  ...
  DEBUG_WM("CAPTIVE PORTAL FALSEEEEEEEEEEEEEEEEEEEEEEEEEE");
  return false;
}
